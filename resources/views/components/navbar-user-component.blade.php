<div class="border-b" x-data="{ isProfileShow: false }">
    <div class="text-gray-700 py-5">
        <div x-data="{ open: false }" class="flex flex-col __container lg:items-center lg:justify-between lg:flex-row">
            ...
            <nav :class="open ? '' : 'hidden'"
                class="flex-col flex-grow pb-4 md:flex md:pb-0 flex md:justify-end md:flex-row mt-5 md:mt-0 ">
                ...
                <div class="flex gap-2 justify-end">
                    ...
                    @auth
                    <div class="relative inline-block lg:mt-0 lg:ml-4 mt-2 w-1/2 md:w-auto">
                        <button @click="isProfileShow = !isProfileShow"
                            class="relative z-10 flex gap-2 items-center md:px-2 w-full md:py-2 px-4 py-3 text-sm text-gray-600 bg-indigo-100 border border-transparent rounded-full focus:border-blue-500 focus:ring-opacity-40 focus:ring-blue-300 focus:ring focus:outline-none">
                            <img class="h-6 w-6 md:h-8 md:w-8 rounded-full object-cover"
                                src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                            <div class="mx-1 md:hidden">
                                <h1 class="text-sm font-semibold text-gray-700">{{ Auth::user()->username }}</h1>
                            </div>
                        </button>
                        <div x-show="isProfileShow"
                            ...
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                @csrf
                                <a class="block px-4 py-3 text-sm text-gray-600 capitalize transition-colors duration-200 transform hover:bg-gray-100"
